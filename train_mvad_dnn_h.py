#!/usr/bin/env python3
"""
train_mvad_dnn_h.py — Train VAD_H_C24_L10 architecture for Multi-Voice Activity Detection

Uses the 1D convolutional encoder-decoder architecture described in
vad_h_c24_l10_architecture.html, adapted for 3-class multivoice VAD:
    0 = silence / no speech
    1 = single speaker
    2 = overlapping speech (≥ 2 speakers)

The network operates directly on 16 kHz raw audio (resampled from 48 kHz
source) and produces frame-level decisions at 250 Hz (64× decimation,
one decision every 4 ms).

Architecture (VAD_H_C24_L10):
    Forward Transform (Encoder, 5 layers, no residual, mixed alignment):
        L1:  Conv1d(1→2,   k=128, s=2) + BN + ReLU  [CENTER]
        L2:  Conv1d(2→4,   k=9,   s=2) + BN + ReLU  [CENTER]
        L3:  Conv1d(4→8,   k=9,   s=2) + BN + ReLU  [CENTER]
        L4:  Conv1d(8→16,  k=9,   s=2) + BN + ReLU  [RIGHT]
        L5:  Conv1d(16→32, k=9,   s=2) + BN + ReLU  [RIGHT]
    Core (7 layers, with residual skip connections, right-aligned):
        L6:  Conv1d(32→32, k=9, s=2)  + BN + ReLU + skip
        L7:  Conv1d(32→32, k=9, s=1)  + BN + ReLU + skip
        L8:  Conv1d(32→32, k=9, s=1)  + BN + ReLU + skip
        L9:  Conv1d(32→32, k=9, s=1)  + BN + ReLU + skip
        L10: Conv1d(32→32, k=9, s=1)  + BN + ReLU + skip
        L11: Conv1d(32→32, k=9, s=1)  + BN + ReLU + skip
        L12: Conv1d(32→32, k=9, s=1)  + BN + ReLU + skip
    Inverse Transform (Decoder, 4 layers, no residual, right-aligned):
        L13: Conv1d(32→16,         k=9,   s=1) + BN + ReLU
        L14: Conv1d(16→8,          k=9,   s=1) + BN + ReLU
        L15: Conv1d(8→4,           k=9,   s=1) + BN + ReLU
        L16: Conv1d(4→num_classes, k=128, s=1) + BN + ReLU
    Output: 3 channels @ 250 Hz (64× decimation from 16 kHz)
...
Audio is resampled to 16 kHz; labels are downsampled to 250 Hz by majority vote.

Usage:
    python3 train_mvad_dnn_h.py
    python3 train_mvad_dnn_h.py --epochs 200 --batch-size 32
    python3 train_mvad_dnn_h.py --augment --overlap-weight-boost 3.0
"""
...
DECIMATION_FACTOR = 64             # total network decimation (2^6)
OUTPUT_RATE_HZ = TARGET_SAMPLE_RATE / DECIMATION_FACTOR  # 250 Hz
FRAME_MS = 1000.0 / OUTPUT_RATE_HZ                      # 4 ms

NUM_CLASSES = 3
CLASS_NAMES = ['Silence', 'Single', 'Overlap']

# Layer definitions: (kernel_size, stride) for the full 16-layer network
LAYER_DEFS = [
    # Forward Transform (Encoder) — L1–L5
    (128, 2),                          # L1:  1→2,   k=128, s=2  [CENTER]
    (9, 2),                            # L2:  2→4,   k=9,   s=2  [CENTER]
    (9, 2),                            # L3:  4→8,   k=9,   s=2  [CENTER]
    (9, 2),                            # L4:  8→16,  k=9,   s=2  [RIGHT]
    (9, 2),                            # L5:  16→32, k=9,   s=2  [RIGHT]
    # Core — L6–L12
    (9, 2),                            # L6:  32→32, k=9, s=2  + residual
    (9, 1),                            # L7:  32→32, k=9, s=1  + residual
    (9, 1),                            # L8:  32→32, k=9, s=1  + residual
    (9, 1),                            # L9:  32→32, k=9, s=1  + residual
    (9, 1),                            # L10: 32→32, k=9, s=1  + residual
    (9, 1),                            # L11: 32→32, k=9, s=1  + residual
    (9, 1),                            # L12: 32→32, k=9, s=1  + residual
    # Inverse Transform (Decoder) — L13–L16
    (9, 1),                            # L13: 32→16, k=9,   s=1
    (9, 1),                            # L14: 16→8,  k=9,   s=1
    (9, 1),                            # L15: 8→4,   k=9,   s=1
    (128, 1),                          # L16: 4→C,   k=128, s=1
]
...
DEFAULT_MODEL_PATH = 'mvad_dnn_h_model.pt'
DEFAULT_HISTORY_PATH = 'mvad_dnn_h_training_history.json'
...
def compute_output_length(input_length):
    """Compute output sequence length analytically for VAD_H_C24_L10.
    ...
    for k, s in LAYER_DEFS:
        ...
        if length <= 0:
            raise ValueError(
                ...
                f"Minimum input: 13,360 samples (0.835 s @ 16 kHz).")
    return length
...
class VAD_H_C24_L10(nn.Module):
    ...
    Implements the architecture from vad_h_c24_l10_architecture.html:
    ...
    Core (7 layers — with residual, right-aligned):
        L6:  Conv1d(32→32, k=9, s=2)  + BN + ReLU + skip
        L7:  Conv1d(32→32, k=9, s=1)  + BN + ReLU + skip
        L8:  Conv1d(32→32, k=9, s=1)  + BN + ReLU + skip
        L9:  Conv1d(32→32, k=9, s=1)  + BN + ReLU + skip
    ...
    Total decimation: 64× (2^6).
    Input:  raw 16 kHz mono audio  →  (B, 1, N_samples)
    Output: num_classes channels @ 250 Hz  →  (B, num_classes, T_out)
    """

    def __init__(self, num_classes=NUM_CLASSES):
        super().__init__()

        # ── Forward Transform (Encoder) — 5 layers ──
        self.fwd = nn.ModuleList([
            ResCbr1dGen(1,  2,  kernel_size=128, stride=2, residual=False),  # L1  [CENTER]
            ResCbr1dGen(2,  4,  kernel_size=9,   stride=2, residual=False),  # L2  [CENTER]
            ResCbr1dGen(4,  8,  kernel_size=9,   stride=2, residual=False),  # L3  [CENTER]
            ResCbr1dGen(8,  16, kernel_size=9,   stride=2, residual=False),  # L4  [RIGHT]
            ResCbr1dGen(16, 32, kernel_size=9,   stride=2, residual=False),  # L5  [RIGHT]
        ])

        # ── Core (with residual connections) — 7 layers ──
        self.core = nn.ModuleList([
            ResCbr1dGen(32, 32, kernel_size=9, stride=2, residual=True),   # L6  (stride-2)
        ...
        # ── Inverse Transform (Decoder) — 4 layers ──
        self.inv = nn.ModuleList([
            ResCbr1dGen(32, 16,         kernel_size=9,   stride=1,
                        residual=False),                                    # L13
            ResCbr1dGen(16, 8,          kernel_size=9,   stride=1,
                        residual=False),                                    # L14
    ...
    def forward(self, x):
        ...
        Returns:
            (B, num_classes, T_out) per-frame logits at 250 Hz
        """
...
def gt_samples_to_frames_250hz(gt_samples, orig_sr, n_frames):
    """
    Convert sample-level GT (at *orig_sr*) to 250 Hz frame-level labels.

    Each 250 Hz frame spans DECIMATION_FACTOR samples at 16 kHz, which
...
def load_split(data_dir, prefix, verbose=True):
    ...
    for idx, wav_path in enumerate(wav_files):
        ...
        frame_labels = gt_samples_to_frames_250hz(gt_samples, sr, n_frames)
...
class MVADChunkDataset(Dataset):
    ...
    Each sample is a (chunk, labels) pair where:
      ...
      - labels: (output_len,) frame-level class labels at 250 Hz
...
def main():
    ap = argparse.ArgumentParser(
        description='Train VAD_H_C24_L10 for Multi-Voice Activity Detection',
    ...
    ap.add_argument('--chunk-samples', type=int, default=DEFAULT_CHUNK_SAMPLES,
                    help='Audio chunk size in samples at 16 kHz (≥ 13360)')
    ...
    # ── Build model ───────────────────────────────────────────────────
    model = VAD_H_C24_L10(num_classes=NUM_CLASSES).to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)

    # ── Banner ────────────────────────────────────────────────────────
    print(f"\n{'═' * 68}")
    print(f"  Multi-Voice VAD — VAD_H_C24_L10 Training")
    print(f"{'═' * 68}")
    print(f"  Device:              {device}")
    print(f"  Data dir:            {data_root}")
    print(f"  Architecture:        VAD_H_C24_L10 (1D Conv Encoder-Decoder)")
